...
import { createCoveragePlan, type CoverageCell, type InventoryEvaluation } from "@/lib/matching/feasibility";
...
const input = (cell = completeCell(), currentSnapshotHash = "a".repeat(64)): PersistedFeasibilityInput => ({
  plan: createCoveragePlan({ id: "plan", snapshotHash: "a".repeat(64), breadth: "ADJACENT_OPPORTUNITIES", requiredFamilyIds: ["family"], cells: [cell], versions: { source: "s", query: "q", inventory: "i", parser: "p", cutoff: "c" }, disposition: "REQUIRED", collisionProof: null }),
  inventory: Array.from({ length: 10 }, (): InventoryEvaluation => ({ eligibility: "ELIGIBLE", sourcePermitted: true, legitimate: true, evidenceSufficient: true, preliminaryUsefulness: true, otherwisePlausible: true })),
  currentSnapshotHash,
  resolutionBlocker: "NONE",
...
function storeFor(payload: PersistedFeasibilityInput) {
  ...
  const store: FeasibilityWorkerStore = {
    async claim(requestId) { events.push("claim"); return { requestId, snapshotId: "snapshot", draftId: "draft" }; },
