...
import { assertPairwiseIndependent, duplicateEdgeReason, selectIndependentInventory } from "@/lib/matching/deduplication";
import { assessFeasibility, coverageComplete, createCoveragePlan, customerFeasibilityMessage, type CoverageCell, type InventoryEvaluation } from "@/lib/matching/feasibility";
...
const plan = (cellChanges: Partial<CoverageCell> = {}, planChanges: Record<string, unknown> = {}) => createCoveragePlan({ id: "plan-1", snapshotHash: "a".repeat(64), breadth: "ADJACENT_OPPORTUNITIES", requiredFamilyIds: ["family-1"], cells: [cell(cellChanges)], versions: { source: "s1", query: "q1", inventory: "i1", parser: "p1", cutoff: "c1" }, disposition: "REQUIRED", collisionProof: null, ...planChanges } as never);
const deliverable = (change: Partial<InventoryEvaluation> = {}): InventoryEvaluation => ({ eligibility: "ELIGIBLE", sourcePermitted: true, legitimate: true, evidenceSufficient: true, preliminaryUsefulness: true, otherwisePlausible: true, ...change });
...
describe("Chunk 3 deduplication and feasibility", () => {
  ...
  it("derives exact LIKELY, LIMITED, and INFEASIBLE count boundaries", () => {
    const likely = assessFeasibility({ plan: plan(), inventory: Array.from({ length: 10 }, () => deliverable()), currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NONE" });
    ...
    const limited = assessFeasibility({ plan: plan(), inventory: [deliverable(), deliverable({ eligibility: "NEEDS_HUMAN_REVIEW", evidenceSufficient: false })], currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NONE" });
    ...
    const infeasible = assessFeasibility({ plan: plan({ outcome: "SUCCEEDED_EMPTY", resultCount: 0 }), inventory: [], currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NONE" });
  ...
  it("counts an allowed employer omission as deliverable and unresolved issues as reviewable", () => {
    const result = assessFeasibility({ plan: plan(), inventory: [deliverable({ eligibility: "ELIGIBLE_WITH_ALLOWED_UNKNOWNS" }), deliverable({ eligibility: "NEEDS_CANDIDATE_INPUT", evidenceSufficient: false })], currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NEEDS_CANDIDATE_INPUT" });
  ...
  it("uses exact reason precedence and does not invent a salary reason without plausible jobs", () => {
    const result = assessFeasibility({ plan: plan(), inventory: [deliverable({ eligibility: "INELIGIBLE", preliminaryUsefulness: false, soleExclusion: "COMPENSATION_UNCONFIRMED" }), deliverable({ eligibility: "INELIGIBLE", preliminaryUsefulness: false, soleExclusion: "QUALIFICATION_GAP" })], currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NONE" });
    expect(result.reasons).toEqual(["QUALIFICATION_GAP", "COMPENSATION_UNCONFIRMED"]); expect(result.primaryReason).toBe("QUALIFICATION_GAP");
    const noPlausible = assessFeasibility({ plan: plan(), inventory: [deliverable({ eligibility: "INELIGIBLE", preliminaryUsefulness: false, otherwisePlausible: false })], currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NONE" });
  ...
  it("marks snapshot mismatch stale and retrieval defects error, never infeasible", () => {
    expect(assessFeasibility({ plan: plan(), inventory: [], currentSnapshotHash: "b".repeat(64), resolutionBlocker: "NONE" }).runState).toBe("STALE");
    expect(assessFeasibility({ plan: plan({ outcome: "TIMEOUT" }), inventory: [], currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NONE" }).runState).toBe("ERROR");
    expect(assessFeasibility({ plan: plan(), inventory: [], currentSnapshotHash: "a".repeat(64), resolutionBlocker: "NONE", expiresAt: "2026-09-04T00:00:00.000Z", now: "2026-09-05T00:00:00.000Z" }).runState).toBe("STALE");
  });
