...
export function careerBreakPresentation(input: {
  ...
  const dates = [input.start, input.end].filter(Boolean).join(" to ");
  return { label, dates };
}
...
export function materialFilename(input: {
  ...
  if (!person || !company || !sanitizeFilenameSegment(input.position)) throw new Error("filename_source_missing");
