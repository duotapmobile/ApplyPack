...
export const typedCriterionSchema = z.discriminatedUnion("kind", [
  ...
  criterionBase.extend({
    ...
    lowerCents: z.number().int().nonnegative().nullable(),
    upperCents: z.number().int().nonnegative().nullable(),
    endpointMeaning: z.enum(["RANGE", "STARTING_AT", "UP_TO", "FIXED"]).default("RANGE"),
    locationApplicability: z.string().trim().min(1).max(300).nullable().default(null),
    basis: z.enum(["BASE", "GUARANTEED_TOTAL", "VARIABLE_OTE"]),
    workerClass: z.enum(["EMPLOYEE", "CONTRACTOR"]),
    source: boundedString,
    comparisonMethod: boundedString,
  }),
