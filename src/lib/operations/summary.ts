...
export function maintenanceHeartbeatIsFresh(
  ...
  return age !== null && Date.parse(lastSucceededAt!) <= now.getTime() && age <= maintenanceMaxAgeSeconds(environment);
}
...
type SourceAuthorityRow = Record<string, unknown>;

// These are the same permission and resource-bound gates used by the schedule
// enqueue function. Legacy job_sources flags cannot authorize a current schedule.
export function scheduledSourceCoverage(
  schedules: SourceAuthorityRow[], sources: SourceAuthorityRow[],
  heads: SourceAuthorityRow[], authorizations: SourceAuthorityRow[],
) {
  const enabled = schedules.filter((row) => row.enabled === true);
  const real = enabled.filter((row) => row.source_id !== SYNTHETIC_SOURCE_ID);
  const bounds = {
    result_bound: "resultBound", page_bound: "pageBound", request_bound: "requestBound",
    response_byte_bound: "responseByteBound", duration_ms_bound: "durationMsBound",
    host_concurrency_bound: "hostConcurrencyBound", quota_unit_bound: "quotaUnitBound",
  };
  const authorized = real.filter((schedule) => {
    const source = sources.find((row) => row.id === schedule.source_id);
    const head = heads.find((row) => row.source_id === schedule.source_id);
    const authorization = authorizations.find((row) => row.id === schedule.source_authorization_id);
    if (!source || source.is_active !== true || !head || !authorization
      ...
      || authorization.source_id !== schedule.source_id
      || authorization.state !== "AUTHORIZED_AUTOMATED" || authorization.access_method !== "AUTOMATED"
      || !Array.isArray(authorization.allowed_actions) || !authorization.allowed_actions.includes("ENUMERATE_JOBS")
      || !Array.isArray(authorization.allowed_hosts) || authorization.allowed_hosts.length === 0) return false;
    const limits = authorization.rate_and_result_bounds as SourceAuthorityRow | null;
    return limits && Object.entries(bounds).every(([field, key]) => {
      const maximum = limits[key];
      return /^\d+$/.test(String(maximum)) && Number.isSafeInteger(Number(maximum))
        && typeof schedule[field] === "number" && Number.isSafeInteger(schedule[field])
        && (schedule[field] as number) >= 0 && Number(maximum) >= (schedule[field] as number);
    });
  });
  return {
    scheduledSources: enabled.length,
    ...sourcePermissionCoverage(real.map((row) => ({ id: row.source_id })), authorized.map((row) => ({ id: row.source_id }))),
  };
}
...
export async function collectOperationsSummary(admin: AdminClient, now = new Date()): Promise<OperationsSummary> {
  ...
  const authorityComplete = authorityResults.every((result) => !result.error
    && Array.isArray(result.data) && result.count === result.data.length);
  const permissionCoverage = scheduledSourceCoverage(schedules.data || [], sourceRows.data || [], heads.data || [], authorizations.data || []);
  ...
  const results = [registeredSources, ...authorityResults,
    syntheticRuns, realRuns, syntheticJobs, realJobs,
