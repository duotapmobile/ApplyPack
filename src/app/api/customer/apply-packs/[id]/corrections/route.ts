...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  await admin.from("apply_pack_items").update({ status: "correction_requested" }).eq("id", id);
  await admin.from("audit_logs").insert({ actor_id: authData.user.id, action: "correction_requested", entity_type: "apply_pack_item", entity_id: id });
