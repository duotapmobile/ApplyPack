...
const lineGenerationSchema = z.object({
  ...
  documentLanguage: z.literal("en-US").default("en-US"),
  requirementMappings: z.array(z.object({
    jobEvidenceId: id,
    classification: z.enum(["DIRECT_EVIDENCE", "TRANSFERABLE_EVIDENCE", "GAP", "UNKNOWN"]),
    candidateFactIds: z.array(id).max(30).default([]),
...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  ...
  if (!/^[0-9a-f]{64}$/i.test(job.content_sha256)
    ...
    || !Object.keys(job.captured_listing).length) {
    return response({ error: "A complete, content-bound job posting is required." }, 409);
  }
  const requirementsResult = await auth.admin.from("ap_requirement_nodes").select("id")
    .eq("job_snapshot_id", job.id).eq("node_kind", "CRITERION").order("position");
  if (requirementsResult.error || !requirementsResult.data?.length) {
    return response({ error: "The current job posting has no complete requirement evidence." }, 409);
  }
  const requirementIds = requirementsResult.data.map((entry) => entry.id);
  const common = {
    contact: {
  ...
  if (input.data.mode === "REFERENCE_REGENERATION") {
    if (!references) return response({ error: "Reference data is unavailable." }, 409);
    const referenceArtifact = await generateEvidenceBoundReferenceSheet({
      ...common,
      references,
      documentLanguage: "en-US",
      requirementMappings: requirementIds.map((jobEvidenceId) => ({
        jobEvidenceId, classification: "UNKNOWN" as const, candidateFactIds: [],
      })),
    ...
    const candidateFactIds = unique([
      ...
      ...input.data.requirementMappings.flatMap((value) => value.candidateFactIds),
    ...
    const mappedRequirementIds = input.data.requirementMappings.map((mapping) => mapping.jobEvidenceId);
    if (mappedRequirementIds.length !== requirementIds.length || unique(mappedRequirementIds).length !== mappedRequirementIds.length
      || !requirementIds.every((requirementId) => mappedRequirementIds.includes(requirementId))) {
    ...
    const generationInput: EvidenceBoundMaterialInput = {
      ...
      requirementMappings: input.data.requirementMappings,
...
async function validateUploadAndRegister(input: {
  ...
  const render = await renderDocumentLocallyForQa({
    ...
    expectedMetadata: input.artifact.metadata,
  });
  ...
  const provenanceChecks = {
    ...
    completeRequirementMap: input.artifact.provenance.requirementMappings.length > 0
