...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  ...
  const { data: line } = await auth.admin.from("ap_material_lines").select("id,purchase_id,active_revision").eq("id", lineId.data).maybeSingle();
    ...
    switch (input.data.action) {
      ...
      case "APPROVE": {
        const { data: file } = await auth.admin.from("ap_generated_file_versions")
          .select("artifact_id,version").eq("id", input.data.fileVersionId).maybeSingle();
        const { data: revision } = await auth.admin.from("ap_material_line_revisions")
          .select("id").eq("line_id", line.id).eq("version", line.active_revision).is("superseded_at", null).maybeSingle();
        const { data: artifact } = file ? await auth.admin.from("ap_generated_artifacts")
          .select("material_line_id,source_line_revision_id,current_file_version,generator_version").eq("id", file.artifact_id).maybeSingle()
          : { data: null };
        if (!artifact || artifact.material_line_id !== line.id
          || !revision
          || artifact.source_line_revision_id !== revision.id
          || Number(artifact.current_file_version) !== Number(file?.version)
          || !isCurrentDocumentGeneratorVersion(artifact.generator_version)) {
          return response({ error: "Regenerate this file under the current document standard before approval." }, 409);
        }
      ...
      case "RELEASE": {
        const { data: revision } = await auth.admin.from("ap_material_line_revisions")
          .select("id").eq("line_id", line.id).eq("version", line.active_revision).is("superseded_at", null).maybeSingle();
        const { data: artifacts } = revision ? await auth.admin.from("ap_generated_artifacts")
          .select("generator_version").eq("material_line_id", line.id)
          .eq("source_line_revision_id", revision.id).is("reference_regeneration_id", null)
          : { data: null };
        if (!artifacts?.length || artifacts.some((artifact) => !isCurrentDocumentGeneratorVersion(artifact.generator_version))) {
          return response({ error: "Regenerate every file under the current document standard before release." }, 409);
        }
      ...
      case "COMMIT_REFERENCE_REGENERATION": {
        const { data: artifacts } = await auth.admin.from("ap_generated_artifacts")
          .select("generator_version").eq("material_line_id", line.id).eq("reference_regeneration_id", input.data.regenerationId);
        if (!artifacts?.length || artifacts.some((artifact) => !isCurrentDocumentGeneratorVersion(artifact.generator_version))) {
          return response({ error: "Regenerate the reference sheet under the current document standard before release." }, 409);
        }
