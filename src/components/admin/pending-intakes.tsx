...
type Snapshot = { id: string; access_email_normalized: string; desired_activities: unknown; avoided_activities: unknown;
  search_breadth: string; guidance_requested: boolean; work_modes: unknown; preferred_work_mode: string | null; us_state_or_dc: string | null;
  employment_types: unknown; preferred_employment_type: string | null; schedules: unknown; benefits: unknown; work_condition_preferences: unknown;
  dealbreakers: unknown; employer_unknown_policy: unknown; salary_hard_minimum_cents: number | null; salary_period: string | null; finalized_at: string };
...
const preferences = (value: unknown) => value && typeof value === "object" && !Array.isArray(value)
  ? Object.entries(value).map(([key, preference]) => `${friendly(key)}: ${friendly(String(preference))}`).join(", ") || "None"
  : "None";
