...
export type MaterialStaffFile = {
  ...
  editableSourceAvailable: boolean;
};
...
export function Chunk5MaterialStaffQueue({ lines }: { lines: MaterialStaffLine[] }) {
  ...
  async function prepare(line: MaterialStaffLine) {
    setBusy(line.lineId);
    try {
      const response = await fetch(`/api/admin/material-lines/${line.lineId}/prepare`, { method: "POST" });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Verified evidence is not ready for drafting.");
      setGenerationJson((current) => ({ ...current, [line.lineId]: JSON.stringify(result.draft, null, 2) }));
      setMessages((current) => ({ ...current, [line.lineId]: "A draft was prepared from verified wording. Review it before generating private files." }));
    } catch (error) {
      setMessages((current) => ({ ...current, [line.lineId]: error instanceof Error ? error.message : "Draft preparation failed." }));
    } finally { setBusy(""); }
  }
  ...
  return <section className="admin-table-wrap" aria-labelledby="chunk5-material-heading">
    ...
    <div className="material-staff-grid">{lines.map((line) => {
      ...
      return <article key={line.lineId}>
        ...
        <details className="staff-review-evidence"><summary>Evidence-bound generation input</summary><button disabled={busy === line.lineId} onClick={() => prepare(line)}>Prepare from verified evidence</button><p>Paste only reviewed content plus candidate-fact and job-evidence IDs. Include every current requirement exactly once in <code>requirementMappings</code> as <code>DIRECT_EVIDENCE</code>, <code>TRANSFERABLE_EVIDENCE</code>, <code>GAP</code>, or <code>UNKNOWN</code>; supported classifications require verified candidate-fact IDs. Contact and permitted reference details are injected server-side.</p><textarea rows={16} spellCheck="false" value={generationJson[line.lineId] || ""} onChange={(event) => setGenerationJson((current) => ({ ...current, [line.lineId]: event.target.value }))} /><button disabled={busy === line.lineId || !line.generationCheckAt} onClick={() => generate(line)}>Generate, scan, and locally render</button></details>
        ...
        {line.files.length ? <div className="material-staff-files">{line.files.map((file) => <article key={file.fileVersionId}><h4>{humanize(file.artifactType)} · version {file.version}</h4><p>{file.filename}</p><p>Automated: {file.automatedPassed ? "passed" : "blocked"} · Approved font: {file.arialResolved ? "resolved" : "blocked"}</p>{file.rendererIdentity ? <p>Renderer: {file.rendererIdentity}</p> : null}<a href={`/api/admin/material-files/${file.fileVersionId}/render-preview`} target="_blank" rel="noreferrer">Open private rendered pages</a>{file.editableSourceAvailable ? <a href={`/api/admin/material-files/${file.fileVersionId}/source-docx`}>Download private editable DOCX source</a> : <p className="match-warning">Editable DOCX source is missing; approval is blocked.</p>}<div className="admin-buttons"><button disabled={busy === line.lineId || !file.automatedPassed || !file.editableSourceAvailable || file.contentApproved || attestation.trim().length < 20} onClick={() => action(line.lineId, { action: "APPROVE", fileVersionId: file.fileVersionId, approvalKind: "CONTENT", attestation })}>Record content approval</button><button disabled={busy === line.lineId || !file.automatedPassed || !file.editableSourceAvailable || file.visualApproved || attestation.trim().length < 20} onClick={() => action(line.lineId, { action: "APPROVE", fileVersionId: file.fileVersionId, approvalKind: "VISUAL", attestation })}>Record visual approval</button></div></article>)}</div> : null}
